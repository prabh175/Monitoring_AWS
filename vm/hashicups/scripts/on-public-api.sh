#!/usr/bin/env bash
# Run on the public-api VM. Calls product-api and payments via Connect upstreams (mTLS).
set -euo pipefail
...
docker run -d --name public-api --restart unless-stopped \
  ...
  hashicorpdemoapp/public-api:latest
...
echo "public-api registered with Connect sidecar (upstreams @ host.docker.internal:19090/19091)"
